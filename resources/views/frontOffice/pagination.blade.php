                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <span class="border-amber-500 text-amber-600 border-t-2 pt-4 px-4 inline-flex items-center text-sm font-medium" aria-current="page">{{ $page }}</span>
                        @else
                            <a href="{{ $url }}" class="border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 border-t-2 pt-4 px-4 inline-flex items-center text-sm font-medium">{{ $page }}</a>
                        @endif
                    @endforeach
                @endif
            @endforeach
