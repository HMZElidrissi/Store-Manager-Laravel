          ...
          <table class="min-w-full divide-y divide-gray-300">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              @foreach ($sales as $sale)
                ...
                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                    <div class="py-1 px-2 rounded border text-center rounded-full text-xs {{ $sale->status == 'delivered' ? 'border-green-500 text-green-500' : ($sale->status == 'pending' ? 'border-amber-500 text-amber-500' : 'border-indigo-500 text-indigo-500') }}">
                        {{ ucfirst($sale->status) }}
                    </div>
                </td>
